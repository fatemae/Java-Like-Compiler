...
static struct S_symbol_ classmarksym = {"<classmark>",0};

void S_beginClassScope(S_table t)
{ S_enter(t,&classmarksym,NULL);
}
...
void S_endClassScope(S_table t)
{S_symbol s;
  do s=TAB_pop(t);
  while (s != &classmarksym);
}
